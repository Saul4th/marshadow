...
TIMER_DURATION = 900  # Default timer duration (1 minute)
...
async def send_timer_notification(participant, channel, message):
    """
    ...
    try:
        ...
        logger.info(f"Sent timer notification DM to {participant.name}, {message[:-2].split("(",1)[1]}")
...
async def start_timer(interaction: discord.Interaction, participant, adjusted_duration=None):
    ...
    logger.info(f"Timer notification thresholds for {participant.name}: halfway={halfway_threshold}s, one-sixth={one_sixth_threshold}s")
    ...
    while remaining_time > 0:
        try:
            ...
            # Update the timer message every second
